...
/*ADD constraint Fk_transaction_products foreign key (product_ids) REFERENCES products (id); omitire esta ya cada transaccion puede tener mas de un producto*/
...
SELECT t.user_id, u.name, count(t.user_id) AS num_trans
FROM transactions t
...
;
   ...
   CREATE TABLE card_status AS -- con este comando creo la tabla a partir de la consulta a continuacion
